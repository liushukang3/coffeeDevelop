<template>
  <van-list class="shopcart" v-model="loadOptions.isLoading" :finished="loadOptions.isFinished" :finished-text="loadOptions.finishedText" @load="loadData" :offset="50">
    <div class="cartEmpty" v-if="shopcartData.length==0">
         <van-empty
          class="custom-image"
          image="https://img.yzcdn.cn/vant/custom-empty-image.png"
           description="购物车还没有东东,赶紧去逛一下吧~"
          />
    </div>
    <div v-else>
        ...
        <van-swipe-cell class="list-items" v-for="(item, index) in shopcartData" :key="index">
          <div class="list-item clearfix">
            ...
            <div class="fl checkbox-item">
              <van-checkbox v-model="item.isCheck" @change="SelectOne" shape="round" />
            </div>
            <!-- 商品信息 -->
      ...
      <!-- 提交订单 -->
      <div v-if="isManage">
        <van-submit-bar class="submit-bar" :price="totalPrice" :disabled="isNotHasCheck" button-text="提交订单" button-type="info" @submit="sumbitOrder">
...
<script>
import "../../assets/css/ShopcartCss.less";
...
export default {
  ...
  methods: {
    ...
    SearchShopcartData() {
      ...
      this.axios({
        ...
        if (result.data.code == 5000) {
          ...
          this.allCartData = result.data.result;

          this.shopcartData  = this.allCartData.splice(0, this.loadCount);
    ...
    loadData() {
      setTimeout(() => {
        this.shopcartData.push(...this.allCartData.splice(0, this.loadCount));
        if (this.allCartData.length == 0) {
